(function () {
	...
	function insertContent(response) {
		...
		for (var i = 0; i < response.length; i++) {
			...
			node.setAttribute("data-puff", response[i].puff);
			node.id = i;

			let child = document.createElement("div");
			child.classList.add("item");
			child.innerHTML = `<img src='src/img/${response[i].image}' alt='img' /><p>${response[i].title}<br>${response[i].year}<br>${response[i].puff}</p>`;
			...
			button.addEventListener('click', (e) => {
				...
				if (tag == '*') {
					...
					items.forEach((item) => {
						...
						if (!item.dataset.tag.includes(tag)) {
		...
		buttons.forEach((button) => {
			...
			button.addEventListener('click', (e) => {
				...
				//sort by puff
					...
					var subjects = document.querySelectorAll('.list-item');
					var subjectsArray = Array.from(subjects);
					let sorted = subjectsArray.sort(comparatorPuff);
					sorted.forEach(e => document.querySelector("#list").appendChild(e));
				}
				else if (sort == 'puff>') {
					var subjects = document.querySelectorAll('.list-item');
					var subjectsArray = Array.from(subjects);
					let sorted = subjectsArray.sort(comparatorPuff).reverse();
					sorted.forEach(e => document.querySelector("#list").appendChild(e));
				}
				else {
	...
	function comparatorPuff(a, b) {
		if (a.dataset.puff < b.dataset.puff) {
			return -1;
		}
		if (a.dataset.puff > b.dataset.puff) {
			return 1;
	...
	search.addEventListener('keyup', (e) => {
		...
		timeout = setTimeout(() => {
			...
			items.forEach((item) => {
				...
				if (!item.dataset.title.toLowerCase().includes(search.toLowerCase()) && !item.dataset.year.toLowerCase().includes(search.toLowerCase())) {
					list.removeChild(item);
				}
			})
		}, 1000);
